...
const convertValuesToPromises = (objects) => {
    if (isEmpty(objects))
        return objects;

    return objects.map(object => isPromise(object)) !== undefined;
};

const processArguments = (args, rawProcessors) => {
    let argumentsProcessors = convertToArray(rawProcessors);
    if (hasPromiseValues(argumentsProcessors)) {
    ...
    checkPromise(checkFn, processors) {
        ...
        return (...args) => {
            ...
            try {
                ...
                argArray = processArguments(args, processors);
