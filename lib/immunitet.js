...
const applyStringProcessors = (argumentValue, processorsList) => {
    return processorsList.reduce((result, processor) => {
        ...
        if (PatternProcessors[processorType] === PATTERN_PROCESSORS.PASS)
            return result;
...
const PatternProcessors = {
    'promise': PATTERN_PROCESSORS.PASS,
...
const hasPromiseValues = (objects, processors) => {
    ...
    let promiseFound = objects.find((object, index) => {
        return ((processors[index] && processors[index] !== 'promise') || !processors[index])
            && isPromise(object);
    });
